...
const AttendanceDonut = ({ summary, height = 260 }: { summary: AttendanceSummary; height?: number }) => {
  const data = [
    { type: 'Có mặt', value: summary.coMat, color: REPORT_COLORS.coMat },
    { type: 'Trễ', value: summary.tre, color: REPORT_COLORS.tre },
    { type: 'Vắng', value: summary.vang + summary.chuaCheckin, color: REPORT_COLORS.vang },
  ...
  return (
    <Pie
      ...
      colorField="type"
      innerRadius={0.6}
      scale={{ color: { domain: data.map((d) => d.type), range: data.map((d) => d.color) } }}
      ...
      annotations={[
        {
          type: 'text',
          style: {
            text: summary.attendanceRate != null ? `${Math.round(summary.attendanceRate * 100)}%` : '—',
            x: '50%',
            y: '50%',
            textAlign: 'center',
            fontSize: 24,
